...
\begin{figure}
  ...
  %\includegraphics[width=0.6\textwidth]{./images/Fig_Psaltis_plot}
    \caption{Astrophysical tests of GR parametrised by the compactness and curvature scales they probe, adapted from \citet{Psaltis2008a}. The dashed line indicates the Schwarzschild radius $r\sub{S} = 2 GM /c^2$ for black holes across a range of masses and the cross indicates the surface of a neutron star.}   
    \label{fig:Psaltis} 
  \end{center}
\end{figure}
Included are:
...
As yet no GWs have been directly detected, although their existence has been inferred from the loss of energy and angular momentum from binary pulsars \citep{Stairs2003}. There are a number of experiments designed to observe GWs \citep{Riles2012}.
...
The maximum mass of a NS depends upon its equation of state. This is currently uncertain and hence there is no definitive theoretical prediction.\footnote{The most famous upper mass is the Tolman--Oppenheimer--Volkoff limit \citep{Tolman1939,Oppenheimer1939}. The model is known to be inadequate as the calculated mass is too small.}
