...
We begin by deriving the Wiener–Khinchin theorem~\citep[chapter 28]{Kittel1958}. For a real signal we must have $\widetilde{x}(f) = \widetilde{x}^\ast(f)$, and since $\widetilde{x}(f) = \widetilde{x}^\ast(-f)$
\begin{equation}
