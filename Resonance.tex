...
\begin{equation}
\tau\sub{res} = \left[\dfrac{2\pi}{\left|\langle\dot{\Omega}(0)\rangle_\Psi\right|}\right]^{1/2}.
\label{eq:t-res}
\end{equation}
Here $\dot{\Omega}(0)$ is the rate of change of $\Omega$ at resonance, which we take to be at $t = 0$. The instantaneous $\dot{\Omega}$ depends upon the orbital phase and oscillates about its mean trend over an orbit (see \figref{wibbles}). We are interested in the averaged behaviour, not the periodic modulations about this, which is why we use the phase-averaged trend $\langle\dot{\Omega}\rangle_\Psi$. Close to resonance, $\Omega$ is well approximated by a first-order Taylor expansion, decreasing linearly with time; hence we make the approximation
\begin{equation}
...
\begin{align}
t\sub{pres} \equiv {} & \tau\sub{pres}(t\sub{pres}) \\
...
\tau_{\mathrm{res},\,s} = {} & \left[\dfrac{2\pi}{\left|\langle s\dot{\Omega}(0)\rangle_\Psi\right|}\right]^{1/2}.
\end{align}
...
\begin{equation}
W\left(\widehat{\lambda}\right) = \intd{0}{\hat{\lambda}}{\exp\left(\frac{is \varpi_1\tau^2}{2}\right)}{\tau}.
\end{equation}
The complex Fresnel integral is
\begin{equation}
\mathcal{Y}(z) = \intd{0}{z}{\exp\left(\frac{i\pi x^2}{2}\right)}{x} = \mathcal{C}(z) + i\mathcal{S}(z),
\end{equation}
where $\mathcal{C}(z)$ and $\mathcal{S}(z)$ are the cosine and sine Fresnel integrals \citep[7.2.7, 7.2.8]{Olver2010}, and hence
\begin{equation}
W\left(\widehat{\lambda}\right) = \sqrt{\frac{\pi}{s\varpi_1}}\mathcal{Y}\left(\frac{s\varpi_1}{\pi}\widehat{\lambda}\right).
\end{equation}
...
Hence,
\begin{equation}
\lim_{|\widehat{\lambda}|\,\rightarrow\,\infty}W\left(\widehat{\lambda}\right) \sim \frac{\sgn \widehat{\lambda}}{\sqrt{2}}\sqrt{\frac{\pi}{|s\varpi_1|}}\exp\left[\sgn(s\varpi_1)\frac{i\pi}{4}\right] + \recip{is\varpi_1}\exp\left(\frac{is \varpi_1 \widehat{\lambda}^2}{2}\right).
\label{eq:Fres-limit}
\end{equation}
...
\begin{equation}
\Psi\left(\widehat{\lambda}\right) = \frac{\Psi_\star}{\eta} + \frac{\varpi_1\widehat{\lambda}^2}{2} + \eta^{1/2}\frac{\varpi_2\widehat{\lambda}^3}{3} + \order{\eta}.
\end{equation}
Using this, the angle variable is
\begin{align}
q\left(\widehat{\lambda};\,\eta\right) = {} & \frac{\Psi_\star}{\eta} + \frac{\varpi_1\widehat{\lambda}^2}{2} + \kappa_\star + \eta^{1/2}{\varpi_2\widehat{\lambda}^3}{3} + \eta^{1/2}g_0^{(1)}(J_\star)\widehat{\lambda} \nonumber \\* 
...
\end{align}
where we have defined $J_\star \equiv J_0\left(\widetilde{\lambda}_\star\right)$ and $\kappa_\star = \kappa_0 + \intd{0}{\widetilde{\lambda}_\star}{g_0^{(1)}[J_0(\tau)]}{\tau}$. The action variable is
\begin{equation}
J\left(\widehat{\lambda};\,\eta\right) = J_\star + \eta^{1/2}G_0^{(1)}(J_\star)\widehat{\lambda} + \frac{\eta^{1/2}}{\varpi_1\widehat{\lambda}}\sum_{s\,\neq\,0}\recip{is}G_s^{(1)}(J_\star)\exp\left[is\left(\frac{\Psi_\star}{\eta} + \frac{\varpi_1\widehat{\lambda}^2}{2}\right)\right] + \order{\eta}.
\end{equation}
...
\begin{align}
\Delta \mathcal{I}_a = {} & \eta\sum_{s\,\neq\,0}F_{a,\,s}^{(1)}(\boldsymbol{\mathcal{I}}_\star)\exp(is \widehat{\kappa}_0)\sqrt{\frac{2\pi}{\left|s \langle\dot{\Omega}\rangle_t\right|}}\exp\left[\sgn\left(s\dot{\Omega}\right)\frac{i\pi}{4}\right] \\
 = {} & \eta\sum_{s\,\neq\,0}F_{a,\,s}^{(1)}(\boldsymbol{\mathcal{I}}_\star)\tau_{\mathrm{res},\,s}\exp(is \widehat{\kappa}_0)\exp\left[\sgn\left(s\dot{\Omega}\right)\frac{i\pi}{4}\right],
\end{align}
using $\boldsymbol{\mathcal{I}}_\star$ for the values on resonance of $E$, $L_z$ and $Q$. Schematically, this can be understood as the magnitude of the forcing function $\sim \eta F_a$ multiplied by the time on resonance $\sim \tau\sub{res}$ and a function that varies with the phase $\widehat{\kappa}_0$. Averaging over all values of $\widehat{\kappa}_0$, is identical to averaging over the $\psi$--$\chi$ $2$-torus and gives an average discrepancy relative to the adiabatic evolution of
\begin{equation}
