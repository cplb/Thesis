...
The geodesic motion can equally be described by $\{E,L_z,Q\}$ or $\{p,e,\theta_-\}$ \citep{Schmidt2002}. Converting between them requires finding the solutions of $V_r = 0$ and $V_\theta = 0$. We employ a slightly different parameter set of $\{p,e,\iota\}$ with inclination \citep{Ryan1996,Glampedakis2002}
\begin{equation}
...
\subsection{Perturbative solution for passage through resonance}
...
\begin{equation}
\tilde{\lambda} = \frac{\lambda}{\eta}.
\end{equation}
The orbits of $q'$ proceed with the fast time $\lambda$, since this is much more rapid than the evolution we are interested in, it is safe to average over it. For this analysis we consider the reduced problem of evolving $q$ and $J$.

At resonance $\tilde{\lambda} = \tilde{\lambda}_\star$ and $\omega(\tilde{\lambda}_\star) = 0$. We assume that the frequency has a simple zero and can be expanded as
\begin{equation}
\omega(\tilde{\lambda}) = \varpi_1(\tilde{\lambda} - \tilde{\lambda}_\star) + \varpi_2(\tilde{\lambda} - \tilde{\lambda}_\star)^2 + \ldots
\label{eq:omega-series}
\end{equation}
The frequency is actually a function of the angle variables, but since these evolve with $\tilde{\lambda}$ it is safe to write it as a function of the slow time.\footnote{In effect we are defining $\omega(\tilde{\lambda}) \equiv \omega[J(\tilde{\lambda})]$.}

Using the slow time, the equations of motion (\ref{eq:Mino-E-o-M}) become
\begin{subequations}
\begin{align}
\diff{q}{\tilde{\lambda}} = {} & \frac{\omega(J)}{\eta} + \sum_s g_s^{(1)}(J)\exp(is q)  + \order{\eta}; \\
\diff{J}{\tilde{\lambda}} = {} & \sum_s G_s^{(1)}(J)\exp(is q) + \order{\eta^2},
\end{align}
\end{subequations}
where we have rewritten the forcing terms as Fourier series. We shall solve these before resonance and then match these to a solution in the transition regime about resonance.

\subsubsection{Solution before resonance}\label{sec:before-res}

To find a solution away from the resonance, we shall decompose the problem to be a function of two time-scales \citep{Kevorkian1971}. We use the slow time $\tilde{\lambda}$ and, as a proxy for the fast time,
\begin{equation}
\Psi = \intd{0}{\lambda}{\omega(\eta\tau)}{\tau} = \recip{\eta}\intd{0}{\tilde{\lambda}}{\omega(\tau)}{\tau}.
\end{equation}
From this
\begin{equation}
...
\begin{subequations}
\begin{align}
q(\lambda;\,\eta) = {} & \Psi + q_0(\tilde{\lambda}) + \eta q_1(\Psi,\tilde{\lambda}) + \order{\eta^2}; \\
J(\lambda;\,\eta) = {} & J_0(\tilde{\lambda}) + \eta J_1(\Psi,\tilde{\lambda}) + \order{\eta^2}.
\end{align}
\end{subequations}
We can also write a series expansion for the frequency
\begin{equation}
\omega(\lambda;\,\eta) = \omega_0((\tilde{\lambda}) + \eta \omega_1(\tilde{\lambda}) + \order{\eta^2}.
\end{equation}
Taking the two time-scales as independent, we may write the time derivative to $\order{\eta}$ as
\begin{equation}
\diffop{\lambda} = \omega_0\partialdiffop{\Psi} + \eta\omega_1\partialdiffop{\Psi} + \eta\partialdiffop{\tilde{\lambda}}.
\end{equation}
...
\begin{align}
\label{eq:q-1}
\omega_0 + \eta\omega_1 + \eta\partialdiff{q_0}{\tilde{\lambda}} + \eta\omega_0\partialdiff{q_1}{\Psi} = {} & \omega(J_0) + \eta\diff{\omega}{J}J_1 + \eta \sum_s g_s^{(1)}(J_0)\exp(is \Psi + Q_0); \\
\eta\partialdiff{J_0}{\tilde{\lambda}} + \eta\omega_0\partialdiff{J_1}{\Psi} = {} & \eta \sum_s G_s^{(1)}(J_0)\exp(is \Psi + Q_0).
\label{eq:J-1}
\end{align}
\end{subequations}
Averaging \eqnref{J-1} over $\Psi$ gives\footnote{The ansatz is constructed such that $J_0 \equive \langle J_0\rangle_\Psi$ and $q_0 \equive \langle q_0\rangle_\Psi$.}
\begin{equation}
\partialdiff{J_0}{\tilde{\lambda} = G_0^{(1)}(J_0).
\end{equation}
This is describes the adiabatic evolution, hence we can identify $J_0(\tilde{\lambda})$ with (the lowest order piece of) the adiabatic solution \citep{Hinderer2008}. If we similarly average \eqnref{q-1} we find
\begin{equation}
\omega_0 + \eta\omega_1 +\eta\partialdiff{q_0}{\tilde{\lambda}} = \omega(J_0) + \eta\partialdiff{\omega}{J}\langle J_1\rangle_\Psi + g_0^{(1)}(J_0).
\end{equation}
...
and hence
\begin{align}
\partialdiff{q_0}{\tilde{\lambda}} = {} & g_0^{(1)}(J_0); \\
...
\begin{equation}
\hat{\lambda} = \frac{\tilde{\lambda} - \tilde{\lambda}_\star}{\eta^{1/2}} = \eta^{1/2}(\lambda - \lambda_\star).
\end{equation}
...
\begin{subequations}
\begin{align}
q(\hat{\lambda};\,\eta) = {} & \hat{q}_0(\hat{\lambda}) + \eta^{1/2} \hat{q}_{1/2}(\hat{\lambda}) + \order{\eta}; \\
J(\hat{\lambda};\,\eta) = {} & \hat{J}_0 + \eta^{1/2} \hat{J}_1(\hat{\lambda}) + \order{\eta}.
\end{align}
\end{subequations}
The series expansion for the frequency, \eqnref{omega-series}, can be rewritten as
\begin{equation}
\omega(\hat{\lambda}) = \eta^{1/2}\varpi_1\hat{\lambda} + \eta\varpi_2\hat{\lambda}^2 + \order{\eta^{3/2}}.
\end{equation}
Proceeding to write the equations of motion in terms of the rescaled time gives
\begin{subequations}
\begin{align}
\diff{q}{\hat{\lambda}} = {} & \varpi_1\hat{\lambda} + \eta^{1/2}\varpi_2\hat{\lambda}^2 + \eta^{1/2}\sum_s g_s^{(1)}(\hat{J}_0,\tilde{\lambda}_\star)\exp(is \hat{q}_0)  + \order{\eta}; \\
\diff{J}{\hat{\lambda}} = {} & \eta^{1/2}\sum_s G_s^{(1)}(\hat{J}_0,\tilde{\lambda}_\star)\exp(is \hat{q}_0) + \order{\eta}.
\end{align}
\end{subequations}

From the equations of motion we can match terms by their order in $\eta^{1/2}$. At zeroth order we find
\begin{equation}
\hat{J}_0 = \hat{\varrho}_0
\end{equation}
is constant, and
\begin{equation}
\hat{q}_0 = \hat{\kappa}_0 + \frac{\varpi_1\hat{\lambda}^2}{2}.
\end{equation}
Using these, we can build the next order terms
\begin{align}
\hat{q}_{1/2} = {} & \hat{\kappa}_{1/2} + \frac{\varpi_2\hat{\lambda}^3}{3} + g_0^{(1)}(\hat{\varrho}_0)\hat{\lambda} + \sum_{s\,\neq\,0}g_s^{(1)}(\hat{\varrho}_0)\exp(is \hat{\kappa}_0)\intd{0}{\hat{\lambda}}{\exp\left(\frac{is \varpi_1\tau^2}{2}\right)}{\tau}; \\
\hat{J}_{1/2} = {} & \hat{\varrho}_{1/2} + G_0^{(1)}(\hat{\varrho}_0)\hat{\lambda} + \sum_{s\,\neq\,0}G_s^{(1)}(\hat{\varrho}_0)\exp(is \hat{\kappa}_0)\intd{0}{\hat{\lambda}}{\exp\left(\frac{is \varpi_1\tau^2}{2}\right)}{\tau};
\end{align}
Both of these involve the complex Fresnel integral, the details of which are given in the following section. We have now constructed the interior solution.

\subsubsection{The complex Fresnel integral}

\begin{equation}
\lim_{\hat{\lambda} \rightarrow -\inft}\intd{0}{\hat{\lambda}}{\exp\left(\frac{is \varpi_1\tau^2}{2}\right)}{\tau} \sim \frac{\sgn \hat{\lambda}}{\sqrt{2}}\sqrt{\frac{\pi}{|s\varpi_1}}}\exp\left[\sgn(s\varpi_1)\frac{\pi}{4}\right] + \recip{is\varpi_1}\exp\left(\frac{is \varpi_1 \hat{\lambda}^2}{2}\right).
\label{eq:Fres-limit}
\end{equation}

\subsubsection{Matching solutions}

To complete our solution we must match the pre-resonance solution of \secref{before-res} with the near resonance solution of \secref{interior-res}. This is achieved by rewriting the before resonance solution in terms of the rescaled time $\hat{\lambda}$ and comparing this with the near resonance solution expanded in the limit of $\hat{\lambda} \rightarrow -\infty$.

To rewrite the pre-resonance solution, we begin with the fast phase parameter
\begin{equation}
\Psi(\hat{\lambda}) = \frac{\Psi_\star}{\eta} + \frac{\varpi_1\hat{\lambda}^2}{2} + \eta^{1/2}\frac{\varpi_2\hat{\lambda}^3}{3} + \order{\eta}.
\end{equation}
Using this, the angle variable is
\begin{equation}
q(\hat{\lambda};\,\eta) = \frac{\Psi_\star}{\eta} + \frac{\varpi_1\hat{\lambda}^2}{2} + \kappa_\star + \eta^{1/2}{\varpi_2\hat{\lambda}^3}{3} + \eta^{1/2}g_0^{(1)}(J_\star)\hat{\lambda} + \frac{\eta^{1/2}}{\varpi_1\hat{\lambda}}\sum_{s\,\neq\,0}\recip{is}g_s^{(1)}(J_\star)\exp\left[is\left(\frac{\Psi_\star}{\eta} + \frac{\varpi_1\hat{\lambda}^2}{2}\right)\right] + \order{\eta},
\end{equation}
where we have defined $J_\star \equiv J_0(\tilde{\lambda}_\star)$ and $\kappa_\star = \kappa_0 + \intd{0}{\tilde{\lambda}_\star}{g_0^{(1)}[J_0(\tau)]}{\tau}$. The action variable is
\begin{equation}
J(\hat{\lambda};\,\eta) = J_\star + \eta^{1/2}G_0^{(1)}(J_\star)\hat{\lambda} + \frac{\eta^{1/2}}{\varpi_1\hat{\lambda}}\sum_{s\,\neq\,0}\recip{is}]G_s^{(1)}(J_\star)\exp\left[is\left(\frac{\Psi_\star}{\eta} + \frac{\varpi_1\hat{\lambda}^2}{2}\right)\right] + \order{\eta}.
\end{equation}
...
\begin{align}
\hat{\kappa}_0 = {} & \frac{\Psi_\star}{\eta} + \kappa_\star; \\
\hat{\varrho}_0 = {} & J_\star.
\end{align}
These fix the integration constants. The more interest result is now found by comparing the $\order{\eta^{1/2}}$ terms. Equating the angle variable expressions igves
\begin{align}
\hat{\kappa}_{1/2} + \sum_{s\,\neq\,0}g_s^{(1)}(\hat{\varrho}_0)\exp(is \hat{\kappa}_0)\left\{\frac{\sgn \hat{\lambda}}{\sqrt{2}}\sqrt{\frac{\pi}{|s\varpi_1}}}\exp\left[\sgn(s\varpi_1)\frac{\pi}{4}\right] + \recip{is\varpi_1}\exp\left(\frac{is \varpi_1 \hat{\lambda}^2}{2}\right)\right\} = {} \recip{is\varpi_1\hat{\lambda}}g_s^{(1)}(\hat{\varrho}_0)\exp\left[is\left(\frac{\Psi_\star}{\eta} + \frac{\varpi_1\hat{\lambda}^2}{2}\right)\right] \nonumber \\
\hat{\kappa}_{1/2} = {} & -\sum_{s\,\neq\,0}g_s^{(1)}(\hat{\varrho}_0)\exp(is \hat{\kappa}_0)\frac{\sgn \hat{\lambda}}{\sqrt{2}}\sqrt{\frac{\pi}{|s\varpi_1}}}\exp\left[\sgn(s\varpi_1)\frac{\pi}{4}\right].
\end{align}
Similarly, for the action variables
\begin{equation}
\hat{\varrho}_{1/2} = {} & -\sum_{s\,\neq\,0}G_s^{(1)}(\hat{\varrho}_0)\exp(is \hat{\kappa}_0)\frac{\sgn \hat{\lambda}}{\sqrt{2}}\sqrt{\frac{\pi}{|s\varpi_1}}}\exp\left[\sgn(s\varpi_1)\frac{\pi}{4}\right].
\end{equation}
We now have a matched solution through resonance.

Having constructed the solution, we see that the lowest order evolution corresponds to the adiabatic solution. The deviations come in at the following order. When we switch from the pre-resonance solution to the post-resonance solution, there is a change in the sign of $\hat{\lambda}$. Therefore, when matching the post-resonance solution $\hat{\varrho}_{1/2}$ and $\hat{\kappa}_{1/2}$ will have also changed sign: there is a change of
\begin{align}
\Delta q = {} & 2 \hat{\kappa}_{1/2}; \\
\Delta J = {} & 2 \hat{\kappa}_{1/2}
\end{align}
across the resonance \citep{Kevorkian1987}. We are not particularly interested in the change in $J$
